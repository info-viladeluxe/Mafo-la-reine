...
## Real date-math bugs found and fixed (2026-08-30)
`src/lib/dateUtils.ts` is now the required way to handle any YYYY-MM-DD date string in this app (`last_period_date`, `lmp_date`, `due_date`, `log_date`, etc.) — use `parseISODateLocal`/`formatISODateLocal`/`addDaysISO`/`daysUntilISO` from it, never `new Date(isoString)` or `.toISOString().slice(0,10)` directly on a date-only value. Two real, user-facing bugs were found and fixed this way:
- **Cycle predictions off by one day for negative-UTC-offset users**: `computeCycleState` parsed `last_period_date` with bare `new Date(string)` (UTC midnight), then compared it against local "today" — for anyone west of UTC, this silently shifted the date back a day, throwing off `dayOfCycle`/`daysUntilPeriod`/fertile-window predictions.
- **Pregnancy week never advanced**: `current_week` was computed once at entry creation and stored statically; the UI displayed that stored value forever instead of recalculating from `lmp_date` — a pregnancy tracker where the week never changes defeats its own purpose. Now computed live from `lmp_date` on every render, falling back to the stored value only if `lmp_date` is missing.
`due_date`/`dueDate` calculations also had the reverse bug on the way out (`.toISOString().slice(0,10)` reads the UTC calendar date, which can be a day off from the local date that was actually meant) — fixed via `addDaysISO`, which stays in local-calendar-date arithmetic throughout.

## Second, more impactful round of date bugs (2026-08-30, same day)
Found `new Date().toISOString().slice(0, 10)` used as "today's date" (form defaults, date-picker `max` attributes) across Medications, Appointments, Symptom, Cycle, Onboarding. This is the *opposite*-direction version of the earlier bug: it reads the UTC calendar date, which is wrong for 1-3 hours after local midnight for any **positive**-UTC-offset user — i.e. most of Africa, Europe, Asia, which given the manifest's own description ("Pensé pour l'Afrique") is presumably most of this app's actual users. During that window, a symptom/medication/appointment/cycle entry logged "for today" would silently get dated yesterday. Fixed with a new `todayISOLocal()` helper in `dateUtils.ts`; also fixed the same `.toISOString().slice(0,10)`-as-Set-key bug in `Symptom.tsx`'s streak counter (same root cause, different symptom). If you add a new "log for today" feature, use `todayISOLocal()` — grep for `toISOString().slice(0, 10)` before adding a new one anywhere in this codebase, since this pattern has now caused two separate real bugs.

## Global error boundary (previously missing — real reliability gap)
Before this, there was NO React error boundary anywhere in the app — any unhandled render/lifecycle error, anywhere in the component tree, crashed the entire app to a blank white screen with no recovery path. `src/components/ErrorBoundary.tsx` (class component, required — no hook equivalent exists) now wraps the whole app in `App.tsx`, outermost (even outside ThemeProvider/I18nProvider/AuthProvider), and shows a friendly branded fallback with a reload button instead. It only catches render-phase errors (per React's error boundary semantics) — it does NOT catch errors in event handlers, async code, or promise rejections; those still need their own try/catch, which is a separate, ongoing concern, not something a single boundary can cover.
