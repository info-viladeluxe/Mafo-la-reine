          ...
          {meds.map((m) => (
            <div key={m.id} className={`card p-4 transition-opacity ${m.is_active ? '' : 'opacity-60'}`}>
              <div className="flex items-start gap-3">
                ...
                <button onClick={() => remove(m.id)} className="grid h-7 w-7 place-items-center rounded-full text-neutral hover:bg-terre-50 hover:text-terre-500 dark:hover:bg-terre-500/10"><Trash2 size={13} /></button>
              </div>
            </div>
          ))}
        </div>
